...
def prepare_for_embedding(df, tokenizer, processed_text_path):
    ...
    for row in df.iterrows():
        ...
        if row[1]["n_tokens"] > MAX_TOKENS_PER_ROW:
            shortened += split_into_many(
                row[1]["text"], tokenizer, max_tokens=MAX_TOKENS_PER_ROW
            )
...
def driver():
    ...
    # read in the file
    df = pd.read_csv(
        os.path.join(processed_text_path, "processed_text.csv"), index_col=0
    )
    ...
    df_shortened.to_csv(
        os.path.join(processed_text_path, "embedding_ready_processed_text.csv"), index=False
    )
